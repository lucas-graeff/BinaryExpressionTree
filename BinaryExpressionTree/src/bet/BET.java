...
public class BET {
		...
		try {
			...
			for (i = 0; i < postfix.length(); i++) {
				...
				if (postfix.charAt(i) == ' ') {
					...
					createSubtree(new BinaryNode(tempValue), 'p');

					// Otherwise, create a variable from the string and push it to the stack
				} else {
					postfixStack.push(new BinaryNode(buildVariable(postfix)));
				}
			}
			...
			for (i = 0; i < infix.length(); i++) {
				...
				if (infix.charAt(i) == ' ') {
					...
					if (infixOperandStack.size() < 2) {
						infixOperatorStack.push(new BinaryNode(tempValue));
					...
					infixOperandStack.push(new BinaryNode(buildVariable(infix)));
				}
			}
	...
	// to the stack
	private void createSubtree(BinaryNode operatorNode, char mode) {
		...
		if (mode == 'p') {
			...
			expressionRoot.setRight(postfixStack.pop());
			// Sets left node equal to the node at the top of the stack
			expressionRoot.setLeft(postfixStack.pop());
			...
			expressionRoot.setRight(infixOperandStack.pop());
			// Sets left node equal to the node at the top of the operand stack
			expressionRoot.setLeft(infixOperandStack.pop());
	...
	private int leafNodes(BinaryNode n) {
	...
	private void printInfixExpression(BinaryNode n) {
		...
		if (n != null) {
			...
			if (n.getValue().charAt(0) == '+' || n.getValue().charAt(0) == '-' || n.getValue().charAt(0) == '*'
					|| n.getValue().charAt(0) == '/') {
				System.out.print("( ");
			}
